...
SPLIT_JSON="$REPO_ROOT/splits/3sd_10x5_single-scale_seed42_paths.json"
RUN_TAG="${1:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="$REPO_ROOT/checkpoints_oc_mappo_top2_plan/phaseD1_E16_B4_A240_M1_10x5_env20_w15_mc2_4seeds/run_${RUN_TAG}"

mkdir -p "$RUN_ROOT"

if [[ ! -f "$SPLIT_JSON" ]]; then
  echo "[prep] split not found, generating fixed 10x5 split: $SPLIT_JSON"
  (
    cd "$REPO_ROOT"

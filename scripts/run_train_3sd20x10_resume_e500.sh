#!/usr/bin/env bash
# 3_SD 20x10, seed=32, 8:1:1, resume from epoch 80 and train to 500 epochs.
set -euo pipefail
...
exec "$PYTHON_BIN" Train_OC_MAPPO.py \
  "${GPU_ARGS[@]}" \
  --split-json splits/3sd_20x10_seed32_split_8_1_1.json \
  --epochs 500 \
  --seed 32 \
  --save-dir exp_runs/s32_3sd20x10_resume_e500 \
  --resume-state-path exp_runs/s32_3sd20x10/train_oc_mappo_state_latest.pt \
  --active-job-rules "" \
